...
- 通信は WebSocket + JSON を共通仕様とし、AIクライアント/観戦ブラウザの両方から接続できるようにする。

## 実装方針（サーバ/クライアント）
- サーバ: C++ 実装で `engine` を利用し、WebSocket 経由で JSON メッセージをやり取りする。
- AIクライアント: Python/C++/Java から同一の WebSocket + JSON 仕様で接続する。
- 観戦: ブラウザから WebSocket で状態更新を受け取り表示する。
